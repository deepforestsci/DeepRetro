...
class TestApiFunctions(unittest.TestCase):
    ...
    def test_clear_molecule_cache_no_molecule_param(self):
        ...
        self.assertEqual(response.json,
                         {"error": "Molecule string is required"})
    ...
    def test_retrosynthesis_default_parameters(self, mock_mol_from_smiles,
    ...
    @patch('src.api.main')
    @patch('src.api.Chem.MolFromSmiles', return_value=True)
    def test_retrosynthesis_custom_parameters_valid(self, mock_mol_from_smiles,
                                                    mock_main,
                                                    mock_save_result):
    ...
    @patch('src.api.main')
    @patch('src.api.Chem.MolFromSmiles', return_value=True)
    def test_retrosynthesis_invalid_model_type_falls_back_to_default(
    ...
    @patch(
        'src.api.save_result'
    )  # Added to avoid UnboundLocalError if main fails before save_result is defined
    @patch('src.api.main')
    @patch('src.api.Chem.MolFromSmiles', return_value=True)
    def test_retrosynthesis_advanced_prompt_false_string(
            self, mock_mol_from_smiles, mock_main, mock_save_result):
    ...
    @patch('src.api.main')
    @patch('src.api.Chem.MolFromSmiles', return_value=True)
    def test_retrosynthesis_main_function_exception_handling(
            self, mock_mol_from_smiles, mock_main, mock_save_result):
        smiles_input = "CCC"
        ...
        self.assertIn("Error in retrosynthesis: Core processing failed",
                      response.json['error'])
    ...
    @patch('src.api.Chem.MolFromSmiles', return_value=True)
    @patch('src.api.clear_cache_for_molecule')
    def test_rerun_retrosynthesis_default_params_success(
            self, mock_clear_cache, mock_mol_from_smiles, mock_main,
            mock_save_result):
    ...
    @patch('src.api.Chem.MolFromSmiles', return_value=True)
    @patch('src.api.clear_cache_for_molecule')
    def test_rerun_retrosynthesis_custom_params_success(
            self, mock_clear_cache, mock_mol_from_smiles, mock_main,
            mock_save_result):
    ...
    @patch('src.api.Chem.MolFromSmiles', return_value=True)
    @patch('src.api.clear_cache_for_molecule')
    def test_rerun_retrosynthesis_main_exception(self, mock_clear_cache,
                                                 mock_mol_from_smiles,
                                                 mock_main, mock_save_result):
        smiles_input = "CCN"
        ...
        self.assertEqual(response.status_code, 500)
        self.assertIn("Error in retrosynthesis: Rerun failed badly",
                      response.json['error'])
        mock_clear_cache.assert_called_once_with(smiles_input)
        mock_save_result.assert_not_called()
    ...
    def test_partial_rerun_missing_smiles_in_payload(self):
        ...
        self.assertTrue("'smiles'" in response.json["error"]
                        or "Error in partial retrosynthesis: 'smiles'"
    ...
    def test_partial_rerun_missing_steps_in_payload(self):
        ...
        self.assertEqual(response.status_code, 500)
        self.assertIn("error", response.json)
        self.assertTrue("'steps'" in response.json["error"]
                        or "Error in partial retrosynthesis: 'steps'"
                        in response.json["error"])
    ...
    def test_partial_rerun_no_stored_result(self, mock_load_result):
        ...
        self.assertEqual(
            response.json,
            {"error": "No previous results found. Run retrosynthesis first."})
    ...
    def test_partial_rerun_step_not_found(self, mock_load_result):
        ...
        self.assertEqual(
            response.json,
            {"error": "Step 3 not found in the synthesis pathway"})
    ...
    def test_partial_rerun_target_step_no_products(self, mock_load_result):
        ...
        self.assertEqual(response.status_code, 400)
        self.assertEqual(response.json,
                         {"error": "Step 1 doesn't have valid products"})

    @patch('src.api.load_result')
    def test_partial_rerun_target_step_invalid_products_format(
    ...
    @patch('src.api.load_result')
    @patch('src.api.Chem.MolFromSmiles', return_value=True)
    def test_partial_rerun_middle_step(self, mock_chem_validate,
                                       mock_load_result, mock_main,
                                       mock_save_result):
        ...
        new_sub_result_for_C = {
            "smiles":
            start_molecule_for_new_synthesis,  # This field in new_result is not actually used by API
            "steps": [{
                "step": 1,
                "name": "New Reaction C->X",
                "reactants": [{
                    "smiles": "C"
                }],
                "products": [{
        ...
        expected_merged_dependencies = {"1": ["2"], "2": ["3"]}

        actual_result = response.json
        self.assertEqual(actual_result['smiles'],
                         original_smiles)  # API returns original target SMILES
        self.assertListEqual(
            sorted(actual_result['steps'], key=lambda x: int(x['step'])),
            sorted(expected_merged_steps, key=lambda x: int(x['step'])))
        self.assertDictEqual(actual_result['dependencies'],
                             expected_merged_dependencies)
        mock_save_result.assert_called_once_with(original_smiles,
                                                 actual_result)
    ...
    @patch('src.api.load_result')
    @patch('src.api.Chem.MolFromSmiles', return_value=True)
    def test_partial_rerun_root_step(self, mock_chem_validate,
                                     mock_load_result, mock_main,
                                     mock_save_result):
        original_smiles = "A"  # Overall target
        original_result = {
        ...
        new_sub_result_for_B = {
            "smiles":
            start_molecule_for_new_synthesis,  # This field is not directly used by API for merging
            "steps": [{
                "step": 1,
                "name": "New B->Z",
                "reactants": [{
    ...
    @patch('src.api.load_result')
    @patch('src.api.Chem.MolFromSmiles', return_value=True)
    def test_partial_rerun_leaf_step(self, mock_chem_validate,
                                     mock_load_result, mock_main,
                                     mock_save_result):
        original_smiles = "A"
        original_result = {
        ...
        new_sub_result_for_C = {
            "smiles":
            start_molecule_for_new_synthesis,
            "steps": [{
                "step": 1,
                "name": "New C->X",
                "reactants": [{
    ...
    @patch('src.api.load_result')
    @patch('src.api.Chem.MolFromSmiles', return_value=True)
    def test_partial_rerun_new_sub_synthesis_empty(self, mock_chem_validate,
                                                   mock_load_result, mock_main,
                                                   mock_save_result):
        original_smiles = "A"
        original_result = {
    ...
    @patch('src.api.load_result')
    @patch('src.api.Chem.MolFromSmiles', return_value=True)
    def test_partial_rerun_main_call_exception(self, mock_chem_validate,
                                               mock_load_result, mock_main,
                                               mock_save_result):
        original_smiles = "A"
    ...
    @patch('src.api.load_result')
    @patch('src.api.Chem.MolFromSmiles', return_value=True)
    def test_partial_rerun_custom_params_to_main(self, mock_chem_validate,
                                                 mock_load_result, mock_main,
                                                 mock_save_result):
        original_smiles = "A"
        # Target step 1 (A->B), so new synthesis will be for "B"
