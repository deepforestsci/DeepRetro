...
results_dir = f"{root_dir}/results/benchmarking/AZ/"
...
async def process_molecule_oursys(mol, folder, run_no, mol_no):
    ...
    try:
        ...
        output_path = f"{results_dir}/{folder}/run_{run_no}/{mol_no}.json"
...
async def process_molecule_az(mol, folder, run_no, mol_no, az_model=None):
    """Process a single molecule asynchronously."""
    molecule = mol
    ...
    try:
        ...
        with ThreadPoolExecutor() as pool:
            res_dict = await loop.run_in_executor(
                pool, 
                functools.partial(process_mol_az_benchmark, molecule, az_model=az_model)
            )
...
async def main_async():
    df_main = pd.read_csv(f"{root_dir}/data/USPTO-50k_100.csv", index_col=0)
    for run_no in range(0, 1):
        # for folder in folder_list:
        for folder in az_model_list:
            ...
            tasks = []
            for i, row in tqdm(df_main.iterrows(), total=len(df_main)):
                mol_no = i
                mol = row['input']
                ...
                # task = asyncio.create_task(process_molecule_oursys(mol, folder, run_no, mol_no))
                task = asyncio.create_task(process_molecule_az(mol, folder, run_no, mol_no, az_model=folder))
                tasks.append(task)
...
def process_mol_az_benchmark(molecule, az_model):
    """Process a single molecule using the AZ model."""
    try:
        solved, res_dict = run_az(molecule, az_model=az_model)
        output_data = format_output(res_dict[0])
        return output_data
    except Exception as e:
        print("Error in molecule:", molecule)
        print("Error:", e)
        return None
