...
folder_list = ["m0p1:Pistachio_100+"]#, "m0p1:Pistachio_50", "m1p0:Pistachio_100+", "m1p0:Pistachio_50"]
# folder_list = ["m1p0:USPTO"]
for i in range(3,6):
    for folder in folder_list:
        if not os.path.exists(f"{root_dir}/results/dfs/{folder}"):
            os.makedirs(f"{root_dir}/results/dfs/{folder}")
        if not os.path.exists(f"{root_dir}/results/dfs/{folder}/run_{i}"):
            os.makedirs(f"{root_dir}/results/dfs/{folder}/run_{i}")
        for mol in mols_dfs:
            molecule = mol
            print(f"Running {mol}")
            ...
            try:
                ...
                with open(f"{root_dir}/results/dfs/{folder}/run_{i}/{mol}_{folder}_3_7.json", "w") as f:
