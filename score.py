...
def score_molecule(smiles: str, sc_config: SCScoreConfig | None = None) -> dict[str, Any]:
    ...
    if canonical is None:
        if _get_chem() is None:
            warning_messages.extend(["RDKit unavailable", "SA_Score unavailable", "SCScore unavailable"])
...
def _get_chem() -> Any | None:
    try:
        # Some broken RDKit/numpy installs print import diagnostics before raising.
        with _quiet_output():
...
def _sc_score_with_error(smiles: str, config: SCScoreConfig | None) -> tuple[float | None, str | None]:
    canonical = canonicalize_smiles(smiles)
    if canonical is None:
        return None, "Invalid SMILES" if _get_chem() is not None else "RDKit unavailable"
...
def _get_scscorer(
    ...
    try:
        ...
        scorer = scorer_class()
        # The standalone model prints during restore; keep library scoring quiet.
        with _quiet_output():
...
def _pathway_result(step_scores: list[dict[str, Any]], error_messages: list[str]) -> dict[str, Any]:
    return {
        "step_scores": step_scores,
        "route_summary": _summarize_steps(step_scores),
        "warnings": [],
...
def _summarize_steps(step_scores: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    complexity_increasing_steps = [
        ...
        if _is_negative(step.get("delta_sa_mean")) or _is_negative(step.get("delta_sc_mean"))
    ]
    ...
    return {
        ...
        "fraction_steps_sa_simplifying": _fraction_simplifying(step_scores, "delta_sa_mean", "sa_simplifies"),
        "fraction_steps_sc_simplifying": _fraction_simplifying(step_scores, "delta_sc_mean", "sc_simplifies"),
...
def _fraction_simplifying(step_scores: list[dict[str, Any]], delta_key: str, simplify_key: str) -> float | None:
    scored_steps = [step for step in step_scores if step.get(delta_key) is not None]
    if not scored_steps:
        return None
    return sum(1 for step in scored_steps if step.get(simplify_key) is True) / len(scored_steps)
