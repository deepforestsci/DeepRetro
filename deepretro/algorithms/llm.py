...
def log_message(message: str, logger=None, file_path: str = "ohuamine_june9.log"):
    if logger is not None:
        logger.info(message)
        with open(file_path, "a", encoding="utf-8") as f:
            f.write(message + "\n")
    else:
        # 'a' opens the file in append mode so you don't overwrite previous logs
        with open(file_path, "a", encoding="utf-8") as f:
            f.write(message + "\n")
...
def obtain_prompt(LLM: str):
    ...
    if advanced_prompt:
        ...
        elif detector[0] in OPENAI_MODELS:
...
def call_LLM(molecule: str,
    ...
    if feedback is not None:
        log_message(f"hallucination feedback added, size={len(feedback)}", logger)
    else:
        feedback = ''
    ...
    if messages is None:
        messages = [{
            ...
            "user",
            "content":
            user_prompt_final.replace('{target_smiles}', build_rdkit_user_prompt(molecule)) + "\n\n" +
            add_on + feedback
        }]
    ...
    try:
        ...
        if response.choices[0].finish_reason == "length" and response.choices[0].message.content is None:
            log_message("Max tokens exceeded before final response", logger)
            raise Exception("Max tokens exceeded before final response")
...
def split_cot_json(res_text: str) -> tuple[int, list[str], str]:
    ...
    try:
        cot_start = res_text.find("<cot>\n")
        cot_end = res_text.find("</cot>")

        if cot_start == -1 or cot_end == -1:
            raise Exception("<cot></cot> tags not found")

        thinking_content = res_text[cot_start + 6:cot_end].strip()
    ...
    try:
        json_start = res_text.find("<json>\n")
        json_end = res_text.find("</json>")

        if json_start == -1 or json_end == -1:
            raise Exception("<json></json> tags not found")

        json_content = res_text[json_start + 7:json_end].strip()
...
def validate_split_json(
    ...
    logger = context_logger.get() if ENABLE_LOGGING else None
    try:
        # result_list = ast.literal_eval(json_content['data'])
...
def build_hallucination_feedback(
    molecule: str,
    hallucinated_pathways: list[list[str]],
) -> str:
    """
    Create a feedback prompt informing the LLM that previously generated
    retrosynthetic pathways were identified as hallucinations.
    ...
    hallucinated_pathways : list[list[str]]
        List of hallucinated pathways. Each pathway is a list containing
        one or more reaction-step reactant strings.

    Returns
    -------
    str
        Feedback prompt for the next LLM iteration.
    """
    lines = [
        "The following retrosynthetic pathways that you previously generated "
        "for the target molecule were evaluated and determined to be hallucinated "
        "(chemically invalid or implausible).",
        "",
        f"Target molecule:",
        f"{molecule}",
        "",
        "Assess the hallucinations in the following generated pathways and avoid reproducing any of these hallucinations below and propose alternative "
    ...
    for pathway_idx, pathway in enumerate(hallucinated_pathways, start=1):
        lines.append(f"Pathway {pathway_idx}:")

        for step_idx, reactant_string in enumerate(pathway, start=1):
            reactants = [
                r.strip()
                for r in reactant_string.split(".")
                if r.strip()
            ]

            lines.append(f"  Step {step_idx} reactants:")
            for reactant_idx, reactant in enumerate(reactants, start=1):
                lines.append(
                    f"    Reactant {reactant_idx}: {reactant}"
                )

        lines.append("")

    lines.extend([
        "Please stick to original Output Requirements.\n",
    ])
    ...
    while (output_pathways == [] and run < max_run):
        ...
        if hallucination_checker is not None:
            log_message(
                ...
                logger)
            status_code, valid_pathways = hallucination_checker(
                molecule, output_pathways)
            if status_code != 200:
                log_message(
                    f"Error in hallucination check: {valid_pathways}",
                    logger)
                run += 0.1
                get_error_log(status_code)
                continue
            if len(valid_pathways) == 0:
                hallucinated_pathways.extend(output_pathways)
                feedback = build_hallucination_feedback(molecule=molecule,
                                             hallucinated_pathways=hallucinated_pathways)
            output_pathways = valid_pathways
