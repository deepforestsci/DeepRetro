"""Recursive retrosynthesis pipeline (ported from src.rec_prithvi)."""
...
def rec_run_prithvi(
    ...
    visited : set, optional
        Set of molecules already processed (for cycle detection).
    depth : int, optional
...
def single_run_DeepRetro(
    ...
    """Single (non-recursive) run of DeepRetro on a molecule."""
    solved, result_dict = run_az(smiles=molecule, az_model=az_model)
    result_dict = result_dict[0]
    logger = context_logger.get()
    logger.info(f"AZ failed for {molecule}, running LLM")
