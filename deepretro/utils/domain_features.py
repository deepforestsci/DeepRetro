...
def extract_domain_features_single(product_smiles: str, reactants_smiles: str) -> np.ndarray:
    """
    ...
    try:
        ...
        p_atoms = Counter(a.GetSymbol() for a in product_mol.GetAtoms())  # element frequency map
        p_bonds = product_mol.GetNumBonds()                                # total bond count
        p_rings = len(Chem.GetSSSR(product_mol))                          # smallest set of smallest rings
        p_arom = sum(1 for a in product_mol.GetAtoms() if a.GetIsAromatic())  # aromatic atom count
        ...
        return np.array([
            ...
            float(r_atoms.get('Cl', 0) - p_atoms.get('Cl', 0)), # chlorine atom delta
            float(r_atoms.get('Br', 0) - p_atoms.get('Br', 0)), # bromine atom delta
            ...
            float(sum(r_atoms.values()) - sum(p_atoms.values())),# total heavy-atom delta
