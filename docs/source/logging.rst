...
:func:`deepretro.configure_logging` once at your application entry point:
...
.. code-block:: python
   ...
   deepretro.configure_logging(level="WARNING")

   # Silence all deepretro logging
   deepretro.configure_logging(level="CRITICAL")

   # Verbose JSON output for production / log aggregation
   deepretro.configure_logging(level="DEBUG", json_output=True)
...
.. autofunction:: deepretro.configure_logging
